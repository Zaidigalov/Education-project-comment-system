class CommentSendModule {
  ...
  reply(button: HTMLElement) {
    let check = button.parentNode!.parentNode!.parentNode!.parentNode?.lastChild?.className;
    if (check !== "tred__typing") {
      const typingArea: HTMLElement = document.createElement("article");
...
export default CommentSendModule;
